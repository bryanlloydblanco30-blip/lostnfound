...
export default function AuthPage() {
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.register(email, srCode, password, email.split("@")[0]);
      router.push("/Main_page");
    } catch (err: unknown) {
      ...
      <div className="relative z-10 flex w-full max-w-[860px] min-h-[575px]">
        ...
        >
             ...
             <div className={`absolute inset-0 px-8 sm:px-12 py-12 transition-all duration-700 overflow-y-auto flex flex-col justify-center ${!isLogin ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100 delay-200'}`}>
                <h1 className="text-3xl md:text-4xl font-extrabold text-[#000000] mb-8 leading-tight text-left">Log In</h1>
                ...
                <form onSubmit={handleLoginSubmit} className="flex flex-col gap-4">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full h-11 bg-[#2c2c2c] text-[#F5F5F5] text-base font-bold rounded-full cursor-pointer transition-all hover:bg-black/90 active:scale-[0.98] mt-2 disabled:opacity-60"
                  >
                    Log In
                  </button>
